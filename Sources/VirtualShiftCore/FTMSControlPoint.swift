...
public struct FTMSControlOwnership: Equatable, Sendable {
    public private(set) var isControlPointSubscribed = false
    public private(set) var hasControl = false

    public init() {}

    public mutating func setControlPointSubscribed(_ subscribed: Bool) {
        isControlPointSubscribed = subscribed
        if !subscribed {
            hasControl = false
        }
    }
    ...
    public mutating func handle(
        ...
        if !isControlPointSubscribed {
            result = .controlNotPermitted
        } else if request == .requestControl {
            hasControl = true
            result = .success
        } else if !hasControl {
            result = .controlNotPermitted
        ...
        return .init(requestOpcode: request.opcode, result: result)
    }

    public mutating func loseControl() -> FitnessMachineStatus? {
