...
/// How well a trainer suits the way Virtual Gears makes gears.
///
/// Virtual Gears shifts by changing the wheel size the trainer is configured
...
public enum TrainerCompatibility: Equatable, Sendable {
    ...
    case supported
    /// Known to shift some other way, so Virtual Gears cannot drive it.
        ...
        if upper.contains("KICKR BIKE") {
            return .unsupported(
                ...
                reason: """
                    ...
                    itself, so it has no use for Virtual Gears.
                    """
            )
        }
